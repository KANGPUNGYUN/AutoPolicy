...
export function GeneratorPage() {
  ...
  useEffect(() => {
    ...
    void CompanyPolicyRepository.getAll().then(setCompanyPolicies)
  }, [])

  const legalTextMap = useMemo(
    () =>
      legalTexts.reduce<Record<string, string>>((acc, cur) => {
      ...
      <section className="form-column">
        ...
        <fieldset className="fieldset">
          ...
          <select
            ...
            onChange={(e) =>
              updatePrivacy({
                retentionPolicy: e.target.value as PrivacyOptions['retentionPolicy'],
              })
            }
          >
      ...
      <section className="preview-column">
        ...
        <div className="reference-panel">
          <div className="reference-header">
            <span className="reference-title">참고 회사 사례</span>
            <select
              value={selectedCompanyId}
              onChange={(e) => setSelectedCompanyId(e.target.value)}
            >
              <option value="">선택 안 함</option>
              {companyPolicies.map((c) => (
                <option key={c.id} value={c.id}>
                  {c.companyName} ({c.serviceType})
                </option>
              ))}
            </select>
          </div>
          {selectedCompany ? (
            <div className="reference-body">
              <h4>이용약관 (KO)</h4>
              <pre>{selectedCompany.termsTextKo || '등록된 내용이 없습니다.'}</pre>
              <h4>개인정보 처리방침 (KO)</h4>
              <pre>{selectedCompany.privacyTextKo || '등록된 내용이 없습니다.'}</pre>
            </div>
          ) : (
            <div className="reference-empty">
              어드민에서 회사 사례를 추가하면 이곳에서 참고용으로 확인할 수 있습니다.
            </div>
          )}
        </div>
      </section>
